...
if(isset($_POST['add_mentor'])){
    ...
    if($count>0){ //This will execute if even a single user with same username is found
        ...
        header("Location: addmentor.php");//This will redirect to the same page after already present username is entered, to avoid the resubmission of same details on reloading the page
        exit;
    }
        ...
        if($result){//If true  boolean value is present in result
            $_SESSION['message']="Mentor added successfully!";//Here we are storing the message in session variable so that we can display the message after the page is redirected to the same page, we can have used alert under echo to display the message at that instant but it will not be displayed as the page is getting redirected to the same page, so we are storing the message in session variable and displaying it after the page is redirected to the same page
            header("Location: addmentor.php");//This will redirect to the same page after adding the mentor to avoid the resubmission of same details on reloading the page
            exit;
        } else {//Boolean value not present in result
            $_SESSION['message']="Failed to add mentor!";//Here we are storing the message in session variable so that we can display the message after the page is redirected to the same page, we can have used alert under echo to display the message at that instant but it will not be displayed as the page is getting redirected to the same page, so we are storing the message in session variable and displaying it after the page is redirected to the same page
            header("Location: addmentor.php");//This will redirect to the same page after mentor upload is failed to avoid the resubmission of same details on reloading the page
            exit;
        }
    }
}
...
<body class="bg-white text-gray-800 flex flex-col min-h-screen">
    ...
    <main class="flex-grow bg-gray-100 p-8">
        <div class="max-w-6xl mx-auto">
            <center><h2 class="text-2xl font-bold mb-6">Add Mentor</h2></center>
            ...
            <div id="form-container" class="mt-8">
                ...
                <label class="block mb-2">User Name:</label>
                        ...
                        <button type="submit" name="add_mentor" class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 transition duration-300">Submit</button>
                    </form>
                </div>
            </div>
        </div>
